    ...
    $.bindViewer = function (elm) {
        ...
        function viewImg() {
            var $obj = $(this),
                ...
                $img = $("<img style='position:absolute;z-index:9999998;left:50%;border-radius:8px;opacity:0;border:2px solid #00BCD4;' src='" + src + "' />"),
                $bg = $("<div style='position:absolute;z-index:9999997;top:0;left:0;width:100%;height:1000%;" + doc_h + "px;background:black;opacity:0.1;'></div>"),
                $close = $("<a title='close' style='position:absolute;z-index:9999999;left:50%;padding:11px 15px;cursor:pointer;background:#0bc6de;color:white;border-radius:50%;font-family:Arial;font-size:14px;transition:background .5s;text-decoration:none;'>X</a>");
            $bg.appendTo("body").hide().fadeIn(200);
            $close.appendTo("body").hover(function () {
                $(this).css({"background": "#D90000", "text-decoration": "none"})
            }, function () {
                $(this).css("background", "#0bc6de")
            }).hide();
