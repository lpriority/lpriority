...
$(document).ready(function(){
	var content ='<div class="circle circle4" style="top:0em;">';
	   for(i=1;i<=32;i++){
		   content+='<li class="c4_sector'+i+'" id="c4c'+i+'_cls" onclick="checkColorPicker()"><span class="c4_text" id="c4c'+i+'">1/32</span></li>';
	   }
	   content+='<div class="circle circle3">';
	   for(j=1;j<=16;j++){
		   content+='<li class="c3_sector'+j+'" id="c3c'+j+'_cls" onclick="checkColorPicker()"><span class="c3_text" id="c3c'+j+'">1/16</span></li>';
	   }
	   content+='<div class="circle circle2">';
	   for(k=1;k<=8;k++){
		   content+='<li class="c2_sector'+k+'" id="c2c'+k+'_cls" onclick="checkColorPicker()"><span class="c2_text" id="c2c'+k+'">1/8</span></li>';
	   }
	...
	if(cId){
		if (cId.match("^c1")) {
			...
			if ($.inArray(cId, c3Array) == -1){
				 c3Array.push(cId);
				 if(c3Array.length == 16){
					 playCorrectSound(); 
				 }
				c3 = c3+1;
			}
		}else if(cId.match("^c4")){
			if ($.inArray(cId, c4Array) == -1){
				 c4Array.push(cId);
				 if(c4Array.length == 32){
					 playCorrectSound(); 
				 }
				 c4 = c4+1;
			}
...
function goForNextGearTest(){
	...
	$.ajax({
		...
		success : function(response) {
				systemMessage("Congratulations! Gear 1 Completed !!");
				$('#mathGameDiv').empty();  
...
var deg = 0;
function slowSpin(){
	var circle = document.getElementsByClassName('circle4');
...
$("#mathGameDiv").dialog({close: function( event, ui ) {location.reload();}});
</script>
<style>
#container {
   position: fixed;
...
<body>
	...
	<table width="100%">
		<tr>
			...
			<div class="button_green"  id="nextTest" onclick="goForNextGearTest()" style="padding-left: 1em;float: right;display:none;">Next Gear</div>
			</td>
		</tr>
		<tr>
