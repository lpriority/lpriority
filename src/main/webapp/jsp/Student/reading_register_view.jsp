	...
	<!-- <tr>
		<td colspan="" width="100%"> 
			 <table width="100%" class="title-pad" border="0">
				<tr>
					<td class="sub-title title-border" height="40" align="left" ><div id="title">View Reading Register</div></td>
				</tr>
			</table>
		</td>
	</tr> -->
	<tr>
		<td style="float:left;padding-left:3.2cm;" align="left" width="90%">
		 <c:if test="${userReg.user.userType == 'student below 13' || userReg.user.userType == 'student above 13' }"> 
			   <input id="bookBank" name="bookBank" value="" type=checkbox class='checkbox-design'  onclick="toggleBankDetails()"  ${ (isNew == false || bookBankChecked == true) ?  'checked':''}> 
		          <label for='bookBank' class='checkbox-label-design'  style="margin-right:63%;font-weight:bold;font-size:20px;cursor:pointer;color: #739005;text-shadow: 0px 1px 0px #80827e;"> <span class="fa fa-university" aria-hidden="true"> &nbsp;<span style="font-weight:bold;font-size:15px;">Book Bank</span></span></label>
          </c:if>
		   <td style="padding-left:1em;float:center" width="10%">
		    ...
		    <c:if test="${userReg.user.userType == 'student below 13' || userReg.user.userType == 'student above 13' }"> 
			    <td style="padding-right:1em;float:center">
			   		<div class="button_green" id="btAdd" name="btAdd" height="52" width="50" onclick="openNewBookDialog(0)" title="Look in the Reading Register Library and see if the book you read is in there. Proceed to add the book if its not found.">Add a book</div>
				</td>
		 	</c:if>
		</td>
	</tr>
...
<c:when test="${fn:length(readRegMasterLt) gt 0 }">
   <tr><td style="padding-top: 1em" align="center" colspan="4">
	    <div id="bankDetails" style="${ (isNew == false || bookBankChecked == true)  ? '':'display:none;'}"> 
		 	<%@include file="reading_register_details.jsp"%>
		</div>
	</td></tr>
	<tr>
