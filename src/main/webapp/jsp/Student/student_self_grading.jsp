...
<html>
<head>
<title><c:choose><c:when test="${(page == 'rflpSelf')}">Fluency Self Review</c:when><c:otherwise>Fluency Peer Review</c:otherwise> </c:choose> </title>
...
<script src="resources/javascript/jplayer/jquery.jplayer.js"></script>
    ...
    <body>
		...
		<tr>
             ...
             <c:choose>
             <c:when test="${gradingTypesId==2}">Fluency Self Assessment</c:when>
              <c:when test="${gradingTypesId==3}">Fluency Peer Review</c:when>
             </c:choose>
              <br> </td>
        </tr>
        </table>
        <table width='100%' class=""><tr>
			...
			<div id="videoDiv">
             ...
             <c:when test="${gradingTypesId==2}">
				<a href="#" onClick="openVideoDialog('student','self_fluency_grading','videoDailog')" class="demoVideoLink"><i class="ion-videocamera" aria-hidden="true" style="font-size:26px;font-weight:bold;color:#e23c00;"></i>&nbsp;video for Fluency Self Assessment</a>
		 	</c:when>
		 	 <c:when test="${gradingTypesId==3}">
		 		<a href="#" onClick="openVideoDialog('student','peer_fluency_grading','videoDailog')" class="demoVideoLink"><i class="ion-videocamera" aria-hidden="true" style="font-size:26px;font-weight:bold;color:#e23c00;"></i>&nbsp;video for Fluency Peer Review</a>
		 	</c:when>
		 	</c:choose>
		 	</div>
			</td>
